...
sc_sdl_log_print(void *userdata, int category, SDL_LogPriority priority,
    ...
    // Suppress unwanted ADB device list logs
    if (strstr(message, " (tcpip) ") != NULL || 
        strstr(message, "ADB device found:") != NULL || 
        strstr(message, "ADB devices over TCP/IP:") != NULL ||
        strstr(message, "ADB devices:") != NULL) {
        return;
    }
