...
scrcpy_ffi_process_execute_p(const char *const argv[], sc_pid *handle, unsigned flags,
    ...
    // Always use CREATE_NO_WINDOW to avoid flashing black cmd/console windows in GUI apps
...
#else
enum sc_process_result
scrcpy_ffi_process_execute_p(const char *const argv[], sc_pid *pid, unsigned flags,
                             sc_pipe *pin, sc_pipe *pout, sc_pipe *perr) {
#undef sc_process_execute_p
    enum sc_process_result sc_process_execute_p(const char *const argv[], sc_pid *pid, unsigned flags,
                                                sc_pipe *pin, sc_pipe *pout, sc_pipe *perr);
    return sc_process_execute_p(argv, pid, flags, pin, pout, perr);
}
#endif